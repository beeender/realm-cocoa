...
public:
    ...
    TransactLogHandler(realm::SharedGroup *sg, NSArray *schema, Func&& func) {
...
void RLMAdvanceRead(realm::SharedGroup *sg, RLMSchema *schema) {
    TransactLogHandler(sg, schema.objectSchema, [](auto&&... args) {
        LangBindHelper::advance_read(args...);
    });
}

void RLMRollbackAndContinueAsRead(realm::SharedGroup *sg, RLMSchema *schema) {
    TransactLogHandler(sg, schema.objectSchema, [](auto&&... args) {
        LangBindHelper::rollback_and_continue_as_read(args...);
    });
}

void RLMPromoteToWrite(realm::SharedGroup *sg, RLMSchema *schema) {
    TransactLogHandler(sg, schema.objectSchema, [](auto&&... args) {
        LangBindHelper::promote_to_write(args...);
    });
}
